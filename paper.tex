...
Here, multiple parallel runs are performed. Within a run, a relative importance score is calculated for each SNP and 
calculated by dividing the importance score for a SNP by a run's minimum importance score. 
...
The relationship between threshold value and false positive rate is unknown. The threshold could be found empirically via permutation 
but the computation cost is high. 


 The greater the score is than one, 
the stronger the evidence is that the SNP is in association with the trait.  Only those SNPs with scores above a certain threshold are 
deemed as being significant. The difficulty is that the relationship between this threshold and false positive rate is unknown. The threshold could be 
found empirically, via permutation but this is computationally demanding.
