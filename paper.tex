...
entire genome can be estimated, simultaneously.  The downside is that it is challenging, analytically, to calculate  
the statistical significance of an SNP effect. We instead calculate significance, empirically, via stability selection (see below). 
%% Permutation has been suggest as an empirical solution to this 
%% limitation \cite{shen2013novel} 
%% but we have instead opted for stability selection (see below).


Of the five multiple-locus methods, MLMM is the closest in philosophy to Eagle. It too is based on  building the 
...
However, there are several differences between the two methods.
 ...
 MLMM and Eagle are both R packages. However, much of Eagle is written in C++ code. 
 
 and they are implemented quite differently leading to substantial 
