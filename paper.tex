...
We were interested in answering the question of how well Eagle finds true SNP-trait associations and avoids false SNP-trait associations.  To 
answer this question, we calculated, empirically, 
the statistical power and false discovery rates of Eagle and the other methods across the six scenarios
For each replicate, we knew which SNPs  had been used to create the quantitative trait data. These SNPs are in true association with 
the trait. By knowing these SNPs, we were able to assess the validity of a  method's findings.  

The power (false discovery rate) of a method, under a particular scenario,  was calculated by taking the median of its power (false discovery rate) 
values across the 100 replicates of the scenario. To calculate the power of a method from its analysis of a replicate, we divided the number of findings for that method that were true by the number of SNP in true association with the trait. To calculate the false discovery rate of a method from its analysis of a replicate, 
we instead divided by the number of  findings for that method. 
A finding was counted as true if the SNP was located within 40 kilobase pairs of a SNP in true 
