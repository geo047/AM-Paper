...
We compared the computational and statistical performance of Eagle against five multiple-locus methods. 
BigRR, LMM-Lasso, and glmnet are all regression-based regularisation 
...
analytically is challenging and is an area of active research \ref{lockhart2014significance}. Instead, we calculate significance empirically via 
...
random forests but where multiple parallel runs are performed. Each run begins with a different random number seed, leading to 
different random forests being formed.  A relative importance score is calculated, within a run,  for each SNP. This is done by dividing a SNP's 
...
and reformulate the (reml) log likelihood for easier computation.  They differ in their estimation procedure. 
FaST-LMM implement's the Brent's algorithm. GEMMA instead implement's the Newton-Raphson algorithm. The  Newton-Raphson is more complicated  but has superior convergence properties to Brent's algorithm. 
Both applications are stand-alone computer programs and in common use.
