...
\begin{abstract}
We present Eagle, a new method for multi-locus association mapping. The motivation for developing Eagle was to make multi-locus association mapping "easy" and the method-of-choice. Eagle's strengths are that it a. is considerably more powerful than single-locus association mapping b. doesn't suffer from multiple testing issues c. is threshold free d. gives results that are immediately interpretable and e. has a computational footprint comparable to single-locus association mapping. By conducting a large simulation study, we will show that Eagle finds true and avoids false SNP-trait associations better than competing single- and multi-locus methods. We also analyse data from a published mouse study. Eagle found over two times more validated findings than the state-of-the-art single-locus method. Eagle has been implemented as an R package, with a web-based Graphical User Interface (GUI) for users less familiar with R.
\end{abstract}
...
that we focused on autosomal SNP and our analyses were based on SNP genotypes  rather than SNP dosages. 
...
same empirical procedure but increased the number of permutations from 100 to 250. 


Eagle was run in three ways.  Eagle chooses the best model via the extended Bayesian information criteria (extBIC) \cite{chen2008extended}. 
...
under this setting (Eagle$^{default}$).  An alternate  \cite{chen2008extended} , less conservative way of setting $\gamma$ is to  let $\gamma = 1 - \frac{1}{(2\kappa)}$ with $\kappa=\frac{log(L)}{log(n_g)}$ where 
...
It is worth noting that the mouse data is similar in size to data generated under the simulation scenario 2000x500K. In the simulation study, Eagle had 
far greater power than the single-locus methods. Yet here, Eagle$^{default}$ found only 37 associations compared to 50 associations found by FaST-LMM.  The reason for the difference is due to Eagle, when run under its default setting, is far more conservative for the real data than the simulation data. Also, here FaST-LMM has been fine-tuned, through permutation,  to have a false discovery rate of 5\%. A fairer comparison is to compare Eagle$^{optimal}$ to FaST-LMM since they have similar false discovery rates. Here, Eagle clearly outperforms FaST-LMM. 
