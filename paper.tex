...
empirically, the significance of the effects. Compared to permutation, the amount of computation is significantly less.  
...
For LMM-Lasso and glmnet, the regularisation parameter was fixed to a "suitable" value. This was done  
via a preliminary analysis of the data where we adjusted the regularisation parameter  to yield
between 20 to 30 SNPs with non-zero effect sizes.  Here, the number of non-zero effects is rather arbitrary and any reasonable 
number of non-zero effects would suffice. 
 We then repeatedly subsampled, without replacement, the data. As recommended \cite{meinshausen2010stability}, we drew 100 subsamples of size $n/2$ from the data. Each subsample was analysed with LMM-Lasso and glmnet, with their regularisation parameters fixed. 
