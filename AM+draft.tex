...
BigRR, LMM-Lasso, and glmnet  implement different regression-based regularisation methods. 
...
samples. When applied to association mapping, they allow the effect of all snp to be modelled simultaneously. 
BigRR implements generalized ridge regression.
In ridge regression, the coefficients of the predictors are shrunk 
to zero by a constant amount.  In generalized ridge regression, the amount of shrinkage is allowed to vary. LMM-Lasso 
implements lasso within a linear mixed model 
...
Glmnet is based on elastic net, a regularisation method that is an improvement over ridge regression and lasso (ref needed). 
All three methods are sophisticated pieces of statistical machinery. The challenge is that while they have the capacity to estimate the 
effect sizes of snp across an entire genome simultaneously, obtaining the statistical significance of these snp is problematic. 
...
Regularisation methods have dominated 
...
reduction which adds to its computational cost. Also, its model building process is different. Eagle 
identifies the $(s+1)$th snp that is in strongest association with the trait by maximising a score statistic. MLMM 
identifies this snp by constructing a separate linear mixed model for each unidentified snp and calculating its statistical significance in
the fixed effects part of the model. The snp with the highest significance is included in the next iteration of the model building process. 

R2VIM is quite different to the other association mapping methods. It implements random forests for association mapping, 
a machine learning algorithm to measure the 
importance of the snp-trait associations. 
There is no relationship between relative importance scores and significance levels. Consequently, 
