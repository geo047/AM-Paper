...
To demonstrate the strengths of Eagle over single-locus association mapping, we analysed data from a large study in outbred mice. We chose this study for several reasons; the data are publicly available, measurements were taken on a large number of traits, and many of the snp-trait associations were validated through other published results. The primary reason for choosing this study though was that its analyses were based on snp dosages (continuous values from zero to one of expected allele counts) rather than snp genotypes. 
Analyses based on dosages rather than genotypes have been shown to have increased power \cite{zheng2011comparison}.  
By repeating their single-locus analyses but with dosages converted into genotypes, we obtained a subset of the original snp-trait findings. 
Whether additional findings by Eagle were true could be determined, if they were found in the original study. 


The performance of Eagle 

We were able to evaluate the performance of Eagle against single-locus association mapping with 
real data. 

Here, we had the unique opportunity of being able to evaluate the performance of Eagle against single-locus association mapping with 
real data. The truth of new findings from Eagle, if found in the original study but now found when the dosage data was converted into 
genotypes, could be assessed. 

Any new findings from Eagle not found by single-locus mapping 

The performance of Eagle could then be compared to single-locus association mapping for real data without the delema of knowing 
whether new findings from Eagle were valid.  We were able to confirm the validity of Eagle's findings through the original findings.  


This then gave us the opportunity that any new findings found by Eagle


When Eagle analyses were performed on the 

 

By repeating  the analyses of REF but with genotypes converted from the dosage data, we obtained single-locus results that were a 
subset of the results found and confirmed in ref. When Eagle was run on the same genotype data and new results not found by the single-locus 
method found, we had the unique opportunity of evaluating the validity of these new results against the original findings for a real data set. 

We were then afforded the opportunity to 




Since our 
focus is on the more commonly used genotypes, the dosages were converted into the discrete values AA, AB, and BB. 



was that it use of snp 
dosages rather than genotypes. 


though was that this study differed from most other studies
in that the analyses were based on snp dosages (continuous values from zero to one of expected allele counts) rather than snp genotypes (Discrete values AA,AB, BB).  Analyses based on dosages rather than genotypes have been shown to have increased power \cite{zheng2011comparison}.  When we repeated the single-locus analyses of ref, by focusing on genoytpes, we found a subset of the previously 
found results due to a loss in information. 



Here, we have a unique opportunity. 


By focusing on genotypes, we had a unique opportunity. 


Since the use of genotypes is more common in association studies than dosages, 
