...
associations which identify $s$ separate genomic regions of interest have been found.  To perform the $s+1$ iteration of 
our model building procedure, we perform the following. 

First, we fit the current model to the data. 
Let $y^{(n \times 1)}$ be the vector containing the quantitative trait data. 
Let $M^{(n \times L)} = [m_1 m_2 \ldots m_L]$ be the matrix containing the genotype data which has been collected 
from $L$ loci that span the genome. Here, the vector $m_j^{(n \times 1)}$ contains the $j$th snp genotypes for the $n$ individuals. 
...
or principal component values to account for population structure (ref). The vector 
...
Up to this point,  this model differs little from standard linear mixed models for association mapping (refs). 
However, 
it is how we specify $u_g$ that distinguishes our model from others. The genetic effects $u_g$ are modelled as 
