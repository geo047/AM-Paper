...
%% Issue 
%%====================
%% Where does us doing stability selection and adding time to jobs come in. 
...
each individual along with a random error.  
 ...
 A replicate was analysed via the different methods as follows. All programs/packages were run at their default settings. 
 The Eagle package and MLMM script were the easiest of the programs/packages to implement. 
...
BigRR, LMM-Lasso, and glmnet  implement regression-based 
regularisation methods. Coefficient estimates of the SNP from fitting all loci simultaneously are reported. However, for these results to be useable, the statistical significance of a
SNP in the model is also required.  The statistical significance rather than the coefficient size of a SNP is a better measure of the strength of evidence for a SNP-trait association. 
Calculating the significance of effects within these regularisation methods is challenging. We resort to 
stability selection, a sampling approach that allows us to calculate 
the statistical significance of the 
SNP empirically (see Methods for details). This does increase the computational burden of implementing these methods for genome-wide association mapping. 

R2VIM implements random forests. As part of analysing data with r2VIM, we were required to specify the minimum size of a terminal node, the number of trees, and number of potential variables 
for building a tree.   HOW TO CHOSE THESE - REASONABLE VALUES ???? LOOK AT PAPERS ... 
...
Of the methods, the findings from Eagle and MLMM are immediately usable. 
Eagle and MLMM, return as their findings
 the set of SNPs in strongest association with the trait. Each 
SNP in this set identifies a separate genomic region of interest, whose position is given by the map location of the SNP.
For the other methods, FaST-LMM and GEMMA report a P value for each SNP as their primary finding. BigRR, LMM-Lasso, and glmnet, after 
stability selection has been performed (see Methods),  report a probability of importance for each SNP. 

FaST-LMM and GEMMA report P values for the statistical significance of a SNP, 

For the other methods, either a P value (.....), relative importance measure (r2VIM), or probability of importance (...,, ..., ...) are reported as the 
findings. Before the results can be interpreted, the snp are placed in map order, peak regions (based on the significance measure) are identified, 
a threshold is set, and a set of SNP findings obtained from identifying the SNP with the largest significance measure in those peak regions above the 
threshold. 
 



findings were reported as either a P value (FaST-LMM and GEMMA), importance 
...
Eagle and MLMM, return as their findings
 the set of SNPs in strongest association with the trait. Each 
SNP in this set identifies a separate genomic region of interest, whose position is given by the map location of the SNP.
BigRR, LMM-Lasso, and glment, after stability selection has been performed (see Methods), 


Eagle and MLMM return as their findings the set of SNPs in strongest association with the trait. Each 
SNP in this set identifies a separate genomic region of interest, whose position is given by the map location of the SNP. 
The other methods required the identification of SNP in 
BigRR, XX, and XX return as their findings 




Interpreting the findings of Eagle and MLMM is trivial. They return as their findings the set of SNPs in strongest association with the trait. Each 
SNP in this set identifies a separate genomic region of interest, whose position is given by the map location of the SNP. 
Interpreting the findings of the other methods is more difficult



Only those SNP found to be in strongest association 

Eagle and MLMM, return as their findings, those SNPs found to be in strongest association with the trait. Each SNP identifies a separate 
genomic region of interest via it's map location. 

The results from Eagle and MLMM required no further processing. Each SNP denotes a separate region of interest where the SNP 
...
When a replicate was analysed, 
 a method's findings were counted as true if the SNPs were located within 40 kilobase pairs of a SNP in true 
