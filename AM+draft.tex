...
Eagle is a new method for multple-locus association mapping. 



Eagle is a new method (and R package) for multiple-locus association mapping. It advances the state of multiple-locus association mapping 
in several ways. First, the results from Eagle are immediately interpretable. They are the set of SNPs in strongest association with the trait where 
each SNP identifies a separate genomic region of interest. Second, Eagle treats association mapping as a model selection instead of a 
variable selection problem. We do not have to content with thresholds and multiple testing issues. 





 It differs from other multiple-locus methods. Its results are 
immediately interpretable,  it can handle much larger datasets due to dimension reduction, and it treats association mapping as a model selection instead of a variable selection problem avoiding threshold and  multiple testing issues. As we saw in the simulation study, Eagle has considerably higher power than single-locus methods but is comparable in run time. The true worth of Eagle is apparent from the analyses of the mouse data. It  found 
more than double the SNP-trait associations than with single-locus association mapping, the method of choice. Furthermore, these extra results 
were not false positives but true verifiable results.  Eagle was built to make multiple-locus association mapping an attractive alternative 
to single-locus association mapping.  
...
In our simulation study, we compared the performance of Eagle against six multiple-locus methods.  We found that Eagle outperforms these methods. 
However, we are cognisant that our deductions are contingent on several implementation choices. We chose to calculate the significance of the 
...
data. Also, when analysing the (sub)samples, calculating a regularisation method's entire solution path is avoided.  Instead,  analyses are 
...
The setting of these parameters greatly affects performance.  We  acknowledge that in the hands of an expert,
 given our resource constraints, the right balance between 
computational and statistical performance could be reached. 
We would like to think that the parameter settings used here are defensible since they match the values used in the original r2VIM publication. 

Eagle's computational speed does come at a cost. It is a weakness shared by all of the methods considered here.  
...
WGAIM, upon which Eagle is based, and RRGAIM are two R packages where this is possible. Here, the difficulty is that for large datasets, 
run time and memory usage become limiting factors for analysis. 
