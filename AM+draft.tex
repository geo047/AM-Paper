...
Suppose a genome-wide association study is conducted. Phenotypic and genotype data are collected. 
...
Also, let $M^{(n \times L)} = [m_1 m_2 \ldots m_L]$ be a matrix of snp genotypes which have been collected from $L$ snp loci 
where $m_j^{(n \times 1)}$ is the vector of snp genotypes collected on the $j$ snp. 
The vector $m_j$  contains values -1, 0, and 1 corresponding to snp genotypes AA, AB, and BB, respectively. 
