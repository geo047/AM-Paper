...
\textbf{glmnet:}  Glmnet is an R package where elastic net regression is implemented. Elastic net is a regularisation method that 
has been applied to generalised linear models \cite{zou2005regularization}.  


Glmnet is a general purpose R package for regression analysis. It implements elastic net, 
a regularisation method where the penalty function 
is a mixture of 
$\ell_1$ (lasso) and $\ell_2$ (ridge regression) normalisation  \cite{zou2005regularization}. 
This allows elastic net to avoid the limitations of lasso (ref) 
...
However, it is included in our list of comparison applications due to its solid performance for genomic prediction (refs).
The model upon which our analyses are based is 
...
However, from exploratory analyses of the simulated data, we found this algorithm yielded a high proportion of false positives. 
...
we included it amongst the applications to benchmark 
Eagle against because of the interest surrounding its use for analysing data from genetic
studies \cite{cho2010joint,huang2015empirical}.  The regression model we used for analysis is 
$y = X \beta + \sum_{l=1}^L m_l \theta_l + e$ where $\theta_l; l=(1,2, \ldots, L)$ is the coefficient for the $l$th snp. 
Elastic net allows us to obtain parameter estimates from this model via a penalty function that is a mixture of 
$\ell_1$ (lasso) and $\ell_2$ (ridge regression). 
The glmnet package 
implements a modification of the LARS algorithm \cite{efron2004least} for finding the optimal regularisation (solution) path. 
However, from exploratory analyses of the simulated data, we found this algorithm yielded a high proportion of false positives. 
We instead fixed the shrinkage parameter to a suitable value 
...
Although glmnet has not been specifically built for association mapping, interest surrounds its use for the analysis of data from 
an association study.  \cite{cho2010joint,huang2015empirical}. 
The analysis model for association mapping 
$y = X \beta + \sum_{l=1}^L m_l \theta_l + e$ where $\theta_l; l=(1,2, \ldots, L)$ is the regression coefficient for the $l$th snp. 
It's penalty function for regularisation is a mixture of $\ell_1$ (lasso) and $\ell_2$ (ridge regression).  
This allows elastic net to avoid some of the
...
implements a modification of the LARS algorithm \cite{efron2004least} for finding the optimal regularisation (solution) path. However, 
