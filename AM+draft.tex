...
the fdr as the significance threshold (and underlying significance level) is adjusted. 
...
mapping though is not only to find true snp-trait associations but to also avoid false snp-trait associations. For those methods with higher power than Eagle, the cost of this higher power is much higher false discovery rates. In other words, these methods are finding more true results 
but they also find many more false results. If we restrict our attention to the power of these methods when the (genome-wide) fdr is less 
than 5\% (see inset plots in Figure X and Sup Figure X), we see that Eagle, closely followed by MLMM,  has the highest power for the lowest 
fdr. This is especially true when Eagle is compared against single-locus methods. For single-locus methods to achieve the same level of power as Eagle, a threshold has to be set where the fdr is extremely high. 
...
If the goal of association mapping was purely to implemented methods with high power, hence having high probability of finding 
snp in true association with a trait, then from Fig ..., it is possible to do this. 


If the goal of association mapping was purely to find snp in true association with a trait, then from Figure X and Sup Figure X,  for those 
methods based on parameter estimation, to 
set a threshold that results in higher power of the method than Eagle. 



From Figure X and Sup Figure X, across the scenarios, it is possible to 
