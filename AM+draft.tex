...
Implementing BigRR, LMM-Lasso, and glmnet required more work. 
The computer programs/packages are based on regularisation methods. Being able to fit all the SNPs simultaneously within a regression 
framework is their strength. The analytical challenges of calculating p-values due to the adaptive nature of the estimation procedure is their 
weakness.  To solve this problem, at the cost of increased computation, we calculated the significance of a SNP empirically via stability selection (see Methods). R2VIM is an R package that implements random forests.  
We were required to 
specify the minimum size of a terminal node, the number of trees, and number of potential variables 
for building a tree. 
