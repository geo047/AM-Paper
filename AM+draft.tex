...
\textbf{r2VIM:} r2VIM is an R package, specifically designed for genome-wide association mapping \cite{szymczak2016r2vim}.
It implements a modified form of random forests. When random forests are applied to association study data in a standard way, 
the result is a ranking of the snp according to their importance values. The difficulty is in knowing how many of the top ranked 
snp are associated, significantly, with the trait. In r2VIM, multiple separate random forest analyses of the data are performed. 


In r2VIM, a strategy is implemented for identifying the snp in strongest association with the trait. 
Here, multiple random forest analyses of the data are performed. For
each analysis, a relative importance score is calculated for each snp based on the minimum observed importance value.  Only 
those snp with large relative importance scores are determined to be in significant association with the trait. 
