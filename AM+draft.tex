...
Eagle is a new method (and R package) for multiple-locus association mapping. It differs from other multiple-locus methods. Its results are 
immediately interpretable,  it can handle much larger datasets due to dimension reduction, and it treats association mapping as a model selection instead of a variable selection problem which avoids multiple testing issues. As we saw in the simulation study, Eagle has considerably higher power than single-locus methods and is comparable in run time. However, the true worth of Eagle is apparent from the analyses of the mouse data. It  found 
