...
We decided on GEMMA and FaST-LMM \cite{lippert2011fast}. These two applications have the same computational complexity (ref), produce exact instead of approximate results, and are highly efficient computationally. They were developed at the same time, independently, but are very similar theoretically. Both perform a single spectral decomposition of the relationship matrix K. Both use the eigenvector matrix to rotate the data. Both reformulate the log likelihood and reml log likelihood into a sum of $n$ terms that are easier to compute. The difference lies in their estimation procedure. FaST-LMM implement's the Brent algorithm to optimise $\delta$. GEMMA instead implement's the Newton Raphson algorithm. Newton Raphson is more complicated in that it also requires the first and second derivatives of a function to be calculated. However, it is superior in terms of its convergence properties to the Brent algorithm (quadratic verse super-linear).  
Both applications are stand along computer programs, popular, and in common use.
